...
export default function Roadmap2({ levels, userAnswers }: RoadmapProps) {
  ...
  // Dummy priority list (later replace with API/localStorage)
  const priorityLevels = ['D1-L0', 'D1-L1']
  ...
  useEffect(() => {
    ...
    // default expansion per step (open if not completed)
  ...
  // Helper for rendering step blocks
  const renderSteps = (list: typeof steps) =>
    list.map((step, index) => {
  ...
  // Split groups
  const prioritySteps = steps.filter((s) => priorityLevels.includes(s.cl_short))
  ...
  return (
    <div className={styles.roadmapContainer}>
      ...
      <div className={styles.groupBlock}>
        <div className={styles.groupHeader} onClick={() => setGroupOpen((p) => ({ ...p, priority: !p.priority }))}>
          <h3>Priority</h3>
          {groupOpen.priority ? <ChevronUp /> : <ChevronDown />}
        </div>
        {groupOpen.priority && <div>{renderSteps(prioritySteps)}</div>}
      </div>
      ...
      {/* Completed group */}
      <div className={styles.groupBlock}>
        <div className={styles.groupHeader} onClick={() => setGroupOpen((p) => ({ ...p, completed: !p.completed }))}>
          <h3>Completed</h3>
          {groupOpen.completed ? <ChevronUp /> : <ChevronDown />}
        </div>
        {groupOpen.completed && <div>{renderSteps(completedSteps)}</div>}
      </div>
      ...
      {/* All group */}
      <div className={styles.groupBlock}>
        <div className={styles.groupHeader} onClick={() => setGroupOpen((p) => ({ ...p, all: !p.all }))}>
          <h3>All</h3>
          {groupOpen.all ? <ChevronUp /> : <ChevronDown />}
        </div>
        {groupOpen.all && <div>{renderSteps(allSteps)}</div>}
      </div>
    </div>
  )
}
